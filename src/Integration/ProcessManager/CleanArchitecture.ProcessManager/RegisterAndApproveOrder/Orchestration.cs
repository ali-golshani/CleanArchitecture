...
internal sealed class Orchestration : TaskOrchestration<ActivityResult, Request>
{
    ...
    public override async Task<ActivityResult> RunTask(OrchestrationContext context, Request input)
    {
        ...
        try
        {
            ...
            for (int tryNumber = 1; tryNumber <= RetryCount; tryNumber++)
            {
                var approveResult = await TryApprove(context, input, tryNumber);
            ...
            return ActivityResult.Failure(["Unreachable Exception"]);
        }
        catch (Exception exp)
        {
