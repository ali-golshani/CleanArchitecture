...
internal class OrderEventsPublisher :
    IntegrationEventsPublisher<OrderDbContext, OrderEventWrapper>
{
    ...
    protected async override Task<OrderEventWrapper?> NextEvent(OrderDbContext db)
    {
        var orderEvent = await
            db.Set<OrderStatusChangedEvent>()
    ...
    private static Task PublishEvent(IServiceProvider serviceProvider, OrderStatusChangedEvent orderEvent)
    {
